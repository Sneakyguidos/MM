...
export class MarketMakerLive {
  ...
  private async handleOrderbookUpdate(orderbook: Orderbook): Promise<void> {
    ...
    try {
      ...
      const midPrice = this.spreadCalculator.getMidPrice(orderbook);
  ...
  private async requote(marketId: number, orderbook: Orderbook): Promise<void> {
    try {
      ...
      const { spread } = this.spreadCalculator.computeDynamicSpread(orderbook);

      // Apply inventory skew
      const { bidPrice, askPrice } = this.inventoryManager.applyInventorySkew(
        this.user,
        marketId,
        midPrice,
        spread
  ...
  private setupPeriodicRequote(marketId: number): void {
    const timer = setInterval(async () => {
      ...
      logger.debug('Periodic requote triggered', { marketId });
      
      // Fetch fresh orderbook would go here in real implementation
